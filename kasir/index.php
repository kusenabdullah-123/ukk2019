...
require '../koneksi.php';
$no = 0;
$sql = 'SELECT `order`.id_order,`order`.tanggal,`order`.keterangan, masakan.nama_masakan, `order`.total FROM `order` INNER join detail_order on `order`.id_order=detail_order.id_order INNER JOIN masakan on masakan.id_masakan=detail_order.id_masakan ';
...
<td><a href="Print.php?id_order=<?php echo $product->id_order; ?>"><i class="fa fa-print"></i> Print</a></td> 
										</tr>
										<?php } ?>
									</tbody>
